...
test('has a working stderr', async (t) => {
  const output = await spawn('ls', ['/non-existent-dir'])
...
test('has a working spawnFile', async (t) => {
  const output = await spawnFile(fixtureBasic, [])
...
test('has a working spawnFile with cwd', async (t) => {
  const output = await spawnFile(fixtureBasic, [], {
